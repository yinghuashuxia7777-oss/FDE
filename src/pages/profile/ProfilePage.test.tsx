...
describe('Capability Profile', () => {
  ...
  it('sorts verified evidence newest first and deduplicates completed challenges', async () => {
    renderProfile(richRepositories());

    const timeline = await screen.findByRole('region', {
      name: 'Verified evidence timeline',
    });
  ...
  it('keeps every capability section accessible and honest when no evidence exists', async () => {
    ...
    expect(
      screen.getByRole('region', { name: 'Verified evidence timeline' }),
    ).toHaveTextContent(/No verified evidence yet/i);
    expect(
