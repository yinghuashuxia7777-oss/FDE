...
describe('Slice A product pages', () => {
  it('makes First Mission the only primary path and defers the regular Dashboard for a new user', async () => {
    ...
    expect(screen.getByRole('region', { name: 'FDE Guide' })).toHaveTextContent(
      'API collaboration boundary',
    );

    expect(
      screen.getByRole('region', { name: 'AI engineering growth journey' }),
    ).toBeVisible();
    expect(
      screen.getByRole('heading', {
        name: 'Become a production AI engineer',
      }),
    ).toBeVisible();
    const capabilityMap = screen.getByRole('figure', {
      name: 'Capability map',
    });
    expect(capabilityMap).toBeVisible();
    expect(
      within(capabilityMap).getByRole('heading', { name: 'Capability map' }),
    ).toBeVisible();
    expect(capabilityMap).toHaveTextContent('Demo Profile');
    expect(capabilityMap).toHaveTextContent('Alex Chen');
    expect(capabilityMap).toHaveTextContent('72% Ready');
    expect(capabilityMap).toHaveAttribute('data-showcase', 'true');
    expect(capabilityMap.querySelectorAll('[data-skill-id]')).toHaveLength(7);
    const orbitRings = capabilityMap.querySelectorAll(
      '.capability-orbit__ring',
    );
    ...
    expect(
      capabilityMap.querySelectorAll('.capability-node__glyph'),
    ).toHaveLength(7);
    expect(
      capabilityMap.querySelectorAll('.capability-node__copy'),
    ).toHaveLength(7);
    expect(
      capabilityMap.querySelector('.capability-map__source'),
    ).toHaveTextContent('Alex Chen');
    expect(capabilityMap).not.toHaveTextContent('No evidence');
    expect(capabilityMap).not.toHaveTextContent(/0 samples?/i);
    expect(
      screen.getByRole('region', { name: 'AI engineering readiness' }),
    ).toHaveTextContent('Insufficient evidence');
    expect(
      screen.getByRole('region', { name: 'Evidence timeline' }),
    ).toHaveTextContent(
      'Complete a case to create reviewable engineering evidence',
    );
    expect(
      screen.getByRole('region', { name: "Today's challenge" }),
    ).toHaveTextContent('Advanced agent incident');
    expect(
      screen.getByRole('complementary', {
        name: 'AI mentor insight',
      }),
    ).toHaveTextContent(/Local rules.*no AI/i);
  ...
  it('keeps a learner with only an in-progress attempt in New User Mode', async () => {
    ...
    expect(readiness).toHaveTextContent('Insufficient evidence');
    expect(readiness).toHaveTextContent('Stage 0');
    expect(
      screen.getByRole('region', { name: 'AI engineering growth journey' }),
    ).toHaveTextContent('Become a production AI engineer');
    expect(
      screen.getByRole('region', { name: 'Evidence timeline' }),
    ).toHaveTextContent(
      'Complete a case to create reviewable engineering evidence',
  ...
  it('composes the AI Growth OS capability command center from real product evidence', async () => {
    ...
    expect(mentor).toHaveTextContent(/Local rules.*no AI/i);
    expect(mentor).toHaveTextContent('Reliability and observability');
    expect(
      within(mentor).getByRole('link', { name: "Start today's challenge" }),
  ...
  it('keeps a historical completed attempt real when active mastery is unavailable', async () => {
    ...
    expect(capabilityMap).toHaveTextContent('Not verified');
  ...
  it('localizes program chrome while preserving Content Pack copy', async () => {
    ...
    expect(
      within(challenge).getByRole('heading', { name: authoredTitle }),
    ).toBeVisible();
    expect(
      within(challenge).getByText('今天继续训练一个尚未完成的 FDE 场景。'),
    ).toBeVisible();
    dashboard.unmount();
  ...
  it('keeps the current challenge visible while onboarding preserves the Foundation-first path', async () => {
    const user = userEvent.setup();
    const source = repositories();
    vi.mocked(source.cases.listActive).mockResolvedValue([
      summary('case-active', {
    ...
    expect(challenge).toHaveTextContent('Active case');
    await user.click(
      screen.getByRole('radio', { name: /become ai engineer/i }),
    );
    await user.click(screen.getByRole('radio', { name: /beginner/i }));
  ...
  it('explains that the first completed case will populate readiness and evidence', async () => {
    const source = repositories();
    render(
      <MemoryRouter>
        <DashboardPage repositories={source} />
    ...
    expect(readiness).toHaveTextContent('Insufficient evidence');
    expect(
      screen.getByRole('complementary', { name: 'AI mentor insight' }),
    ).toHaveTextContent(/complete a real case to unlock a local diagnosis/i);
    expect(
      screen.getByRole('region', { name: 'Evidence timeline' }),
    ).toHaveTextContent(
      /complete a case to create reviewable engineering evidence/i,
    );
