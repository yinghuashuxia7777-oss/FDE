...
describe('NewUserLearningJourney', () => {
  ...
  it('requires visiting the matching Foundation before completing a First Mission', async () => {
    ...
    expect(
      screen.getByRole('group', { name: 'Choose your growth goal' }),
    ).toBeVisible();
    expect(
    ...
    expect(
      screen.queryByRole('region', { name: 'FDE Guide' }),
    ).not.toBeInTheDocument();
  ...
  it('keeps profile setup visible for a learner with only an in-progress attempt', () => {
    render(
      <I18nProvider initialLanguage="en-US">
        <MemoryRouter>
          <LearningJourneyProvider>
  ...
  it('does not force a learner with a completed attempt through profile setup again', () => {
    render(
      <I18nProvider initialLanguage="en-US">
        <MemoryRouter>
          <LearningJourneyProvider>
  ...
  it('keeps focused profile setup usable while the optional Concept sidecar is unresolved', () => {
    ...
    expect(
      screen.queryByRole('region', { name: 'FDE Growth Roadmap' }),
    ).not.toBeInTheDocument();
  });
  ...
  it('does not recommend an unrelated Concept as the mission guide', async () => {
    ...
    expect(
      screen.getByRole('region', { name: 'Your First Mission' }),
    ).toBeVisible();
    expect(screen.queryByText('Unrelated concept')).not.toBeInTheDocument();
  });
});
