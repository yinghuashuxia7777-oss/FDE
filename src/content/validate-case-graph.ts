import type { CaseNode, FdeCase, NodeSubmission } from '../domain/cases/types';
import { evaluateNode } from '../domain/scoring/evaluate-node';
...
export function compareContentIssues(
  left: ContentIssue,
  right: ContentIssue,
): number {
  return (
...
export function validateCaseGraph(
  ...
  for (const component of stronglyConnectedComponents(reachable, edges)) {
    ...
    const isCycle =
      component.length > 1 ||
      (edges.get(firstNodeId) ?? []).includes(firstNodeId);
  ...
  return issues.sort(compareContentIssues);
}
